...
for ii = 13:14
    NA(ii-12) = XRDMLread(sprintf('./%s/NA Wiley_quick gonio scan_4.xrdml',directory(ii).name));
    ANNEAL(ii-12) = XRDMLread(sprintf('./%s/110C Wiley_quick gonio scan_4.xrdml',directory(ii).name));    
end
...
hold on
plot(NA(8).Theta2,NA(8).data)
plot(NA(4).Theta2,NA(4).data)
title('Simultaneous x2 No Anneal XRD')
legend('Uncontrolled','25C','75C')
xlim([NA(1).Theta2(1) NA(1).Theta2(779)])
savefig('./processed_imgs/Simultaneous_XRD2')
print('./processed_imgs/Simultaneous_XRD2','-dpng','-r300','-f1')
hold off
