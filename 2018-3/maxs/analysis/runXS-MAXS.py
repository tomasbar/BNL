...
mask_dir = SciAnalysis_PATH + '/SciAnalysis/XSAnalysis/masks/Dectris/'
detector = 'MAXS'

if detector == 'SAXS':
    # MAXS detector on CMS
    #calibration = Calibration(wavelength_A=0.9184) # 13.5 keV
    calibration = Calibration(wavelength_A=12.398/11) # 11 keV
    ...
    #for 5m setup
    calibration.set_beam_position(468, 619-16)
    calibration.set_distance(0.152)
...
protocols = [
    ...
    Protocols.q_image(blur=1.0, bins_relative=0.5, plot_range=[-3.2, 0.1, 0, 3.2], _xticks=[0, 1.0, 2.0, 3.0], ztrim=[0.2, 0.01]) ,
    ##Protocols.qr_image(blur=1.0, bins_relative=0.5, plot_range=[-0.1, 3.0, 0, 3.0], _xticks=[0, 1.0, 2.0, 3.0], zmin=1010., ztrim=[None, 0.01]) ,
    Protocols.qr_image(blur=None, bins_relative=0.8, plot_range=[-3.0, 0.1, 0, 3.0], _xticks=[0, 1.0, 2.0, 3.0], ztrim=[0.38, 0.002], dezing_fill=True) ,
    ...
    Protocols.thumbnails(crop=None, resize=1, cmap=cmap_vge_hdr, ztrim=[0.005, 0.01]) ,
